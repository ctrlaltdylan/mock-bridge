...
if (!PORT)
    throw new Error('SHOPIFY_PORT is not set');
const DEBUG = process.env.SHOPIFY_DEBUG;
if (!DEBUG)
    throw new Error('SHOPIFY_DEBUG is not set');
const MOCK_USER_ID = process.env.MOCK_USER_ID;
if (!MOCK_USER_ID)
    throw new Error('MOCK_USER_ID is not set');
const server = new _1.MockShopifyAdminServer({
    appUrl: APP_URL,
    clientId: CLIENT_ID,
